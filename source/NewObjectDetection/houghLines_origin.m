...
%         frame_mat = imgaussfilt(frame_mat, 2);
        ...
        0.2*max(H(:))
        P  = houghpeaks(H,100,'threshold',ceil(0.1*max(H(:)))); % get hough peak value
        lines = houghlines(BW,T,R,P,'FillGap',10,'MinLength',50); % get lines from hough tranform by configuring some parmetters 
